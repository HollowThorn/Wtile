...
- **Tag rules** (dwm's `rules[]`): map windows to a tag by process name, class, or title regex, so
  an app always opens on its own tag -- optionally switching the view there as it appears. A rule
  beats remembered placement for that window.
